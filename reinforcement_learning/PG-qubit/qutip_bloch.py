...
expectation_value = lambda op, st: (st.conj() @ op @ st).real 
...
class Bloch:
    ...
    def add_states(self, state, kind='vector', colors=None, alpha=1.0):
        ...
        for k, st in enumerate(state):
            vec = [expectation_value(sigmax, st),
                   expectation_value(sigmay, st),
                   expectation_value(sigmaz, st)]
            if kind == 'vector':
    ...
    def add_annotation(self, ann_pos, text, **kwargs):
        """
        ...
        self.annotations.append({'position': ann_pos,
                                 'text': text,
                                 'opts': kwargs})
    ...
    def add_arc(self, start, end, fmt="b", steps=None, **kwargs):
        ...
        **kwargs : dict
        ...
        pt1 = [
            expectation_value(sigmax, start),
            expectation_value(sigmay, start),
            expectation_value(sigmaz, start),
        ]

        pt2 = [
            expectation_value(sigmax, end),
            expectation_value(sigmay, end),
            expectation_value(sigmaz, end),
        ]
    ...
    def save(self, name=None, format='png', dirc=None, dpi=None):
        ...
        if dpi:
            self.fig.savefig(complete_path, dpi=dpi)
        else:
