...
def Bloch_plots(states, times, target, file_name='test', movie_frames=False):
	...
	for i, psi in enumerate(states):
		...
		ann_str="$\mathcal{F}(t{=}"+"{0:.2f})".format(times[i])+"{=}"+"{0:.3f}$".format(np.around(Fidelity[i],3) )
		bloch.add_annotation(ann_pos, ann_str, fontsize=20)
...
def Bloch_movie(states, times, target, movie_name='test'):
	...
	frame_name = movie_name+'_frame'
	Fidelity = Bloch_plots(states, times, target, file_name=frame_name, movie_frames=True)

	# create movie
	frame_dir = my_dir + '/movie_frames/'
	cmd = "ffmpeg -y -framerate 5 -i " + frame_dir+frame_name+"_%01d.png -c:v libx264 -r 30 -pix_fmt yuv420p " + 'bloch_plots/'+movie_name+".mp4"
	print(cmd)
	# execute command cmd
	os.system(cmd)
